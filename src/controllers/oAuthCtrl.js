...
const googleAuthCallback = (req, res, next) => {
  passport.authenticate('google', { session: false }, async (err, user, info) => {
    ...
    try {
      ...
      console.log(
        `Google Auth Callback: User profile received - ID: ${googleIdFromPassport}, Email: ${emailFromPassport}, Name: ${nameFromPassport}`
      );
      ...
      console.error(
        'Google Auth Callback: Internal server error during token generation or user sync:',
        error
      );
      res.redirect(
